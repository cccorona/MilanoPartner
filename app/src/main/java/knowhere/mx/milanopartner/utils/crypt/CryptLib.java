package knowhere.mx.milanopartner.utils.crypt;
...
public class CryptLib {
    ...
    private final static String TAG = "CryptLib";
    // private final static String encodeKey = "UTF-8";
    private final static String encodeKey = "ISO-8859-1";
    ...
    public static final String md5(final String inputString) {
        try {
            MessageDigest digest = MessDigest();
            ...
            MilanoLogger.getINSTANCE().escribeArchivo(TAG, "Error: emedecinco", e.getMessage());
        }
        return "";
    }

    private static MessageDigest MessDigest() throws NoSuchAlgorithmException {
    ...
    public static String SHA256(String text, int length) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        md.update(text.getBytes(encodeKey));
    ...
    private String encryptDecrypt(String _inputText, String _encryptionKey, EncryptMode _mode, String _initVector)
        ...
        int len = _encryptionKey.getBytes(encodeKey).length; // length of the
        // key provided

        if (_encryptionKey.getBytes(encodeKey).length > _key.length)
            len = _key.length;

        int ivlen = _initVector.getBytes(encodeKey).length;

        if (_initVector.getBytes(encodeKey).length > _iv.length)
            ivlen = _iv.length;

        System.arraycopy(_encryptionKey.getBytes(encodeKey), 0, _key, 0, len);
        System.arraycopy(_initVector.getBytes(encodeKey), 0, _iv, 0, ivlen);
        ...
        if (_mode.equals(EncryptMode.ENCRYPT)) {
            ...
            // Read
            // Globalmembers.WEB_PROTOCOL_SEC +
            ...
            // cipher instance
            byte[] results = _cx.doFinal(_inputText.getBytes(encodeKey)); // Finish
            // UTF-8
            // ISO-8859-1
            // multi-part
            // transformation
        ...
        if (_mode.equals(EncryptMode.DECRYPT)) {
            ...
            _out = new String(decryptedVal, 0, decryptedVal.length, encodeKey);
        }
